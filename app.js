...
// CRITICAL FIX 1: PluckSynth monophonic → ใช้ 4 strings แยกกัน (1 ต่อ 1 note)
// PluckSynth ตัวเดียว triggerAttack note ใหม่ตัดเสียงเก่า → ได้แค่ note สุดท้าย
// ใช้ 4 ตัวแยกกัน → chord เสียงครบทุก note
let strings = [];  // array ของ PluckSynth, 1 ตัวต่อ 1 สาย
...
async function ensureAudio() {
  ...
  try {
    ...
    throw e; // HIGH FIX: re-throw → triggerCountdown .catch() ถูกเรียก → countdown ไม่เริ่มถ้า audio fail
  }
}
...
async function initAudio() {
  try {
    ...
    masterVol = new Tone.Volume(-3).connect(reverb);

    // CRITICAL FIX 1: สร้าง 4 PluckSynth แยกกัน = 4 สายกีต้าร์อิสระ
    // แต่ละตัวรับ note คนละตัว → เสียง chord ครบทุก note พร้อมกัน
    strings = Array.from({ length: 4 }, () =>
      new Tone.PluckSynth({
        attackNoise: 1.2,
        dampening:   3500,
        resonance:   0.96
    ...
    console.log("✅ Audio: 4 strings ready");
    setStatus("🎸 กด เปิดเสียง แล้วชู 5 นิ้วเพื่อเริ่ม");
  } catch(e) {
    audioInitDone = false;
    strings = [];
    showError("❌ Audio init: " + e.message);
    throw e; // CRITICAL FIX 2: re-throw → ensureAudio catch → audioReady ไม่ set
              // → triggerCountdown .catch() ถูกเรียก → countdown ไม่เริ่มเมื่อ audio พัง
  }
}

// ฟัง event แบบ non-once เพื่อ retry ได้ถ้า initAudio fail ครั้งแรก
function _onUserInteract() {
  if (!audioReady) ensureAudio().catch(() => {}); // LOW FIX: catch unhandled rejection
}
...
async function playStrum(velocity) {
  if (!strings.length || !masterVol) return;
  if (!audioReady) return;
  if (Tone.context.state === "suspended") {
    try { await Tone.context.resume(); } catch(e) { return; }
  }
  if (Tone.context.state !== "running") return;
  const notes = CHORDS[currentChord];
  if (!notes) return;
  masterVol.volume.cancelScheduledValues(Tone.now());
  masterVol.volume.rampTo(-18 + velocity * 18, 0.04); // MED FIX: -18dB→0dB dynamic range กว้างขึ้น
  const spread = 0.018 + (1 - velocity) * 0.05;
  const t = Tone.now();
  // CRITICAL FIX 1: ดีดแต่ละ note ด้วย string อิสระ → chord เสียงครบทุก note
  notes.forEach((note, i) => {
    strings[i % strings.length].triggerAttack(note, t + i * spread);
  });
}
...
async function retryInit() {
  ...
  countdownActive = false;
  // cleanup strings
  strings.forEach(s => { try { s.dispose(); } catch(e) {} });
  strings = [];
...
function onResults(gestResult, now) {
  ...
  for (let i = 0; i < handCount; i++) {
    ...
    if (i === chordIdx && !five) {
      ...
      const strumMotion = (handCount === 1 && Math.abs(hState[i].dy) > 0.035);
      ...
      if (!strumMotion) {
        ...
        // MED CHORD FIX: chordFromFingers เฉพาะเมื่อ gesture ชัดเจน (ไม่ใช่ None)
        // เดิม: !(None && score<0.3) → None score=0.5 ก็ผ่าน → chord กระพริบ
        // ใหม่: รัน chordFromFingers เฉพาะ gesture ที่ไม่ใช่ None ที่มี confidence ดีพอ
  ...
  // Strum — HIGH FIX: threshold ลดจาก 0.055 → 0.022 สำหรับ l[0].y (ข้อมือ เคลื่อนน้อยกว่านิ้วชี้)
  // MED FIX: velocity divisor /1.2 (เดิม /4), volume range -18+vel*18 (เดิม -6+vel*6)
