...
async function initAudio() {
  try {
    ...
    // Chain: guitarSynth → masterVol → reverb → Destination
    reverb = new Tone.Reverb({ decay: 2.0, wet: 0.3 });
    await reverb.generate();   // pre-compute IR (~0.5-2s)
    reverb.toDestination();

    masterVol = new Tone.Volume(0).connect(reverb);

    // PolySynth: polyphonic
    // MED FIX: release ลดจาก 1.5s → 0.5s ป้องกัน voice-steal (4 notes × 1500ms/130ms > 32)
    // LOW FIX: volume 0 (จาก -6) + maxPolyphony=16 → เสียงดังขึ้น ไม่ steal
...
async function retryInit() {
  ...
  guitarSynth = null; reverb = null; masterVol = null;
...
async function startRecording() {
  // LOW FIX: stop recorder เก่าก่อน ป้องกันกดซ้ำ → chunks หาย
